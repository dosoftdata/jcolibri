package info.dosoft.jcolibri.web.rest;
...
import info.dosoft.jcolibri.config.KafkaProperties;
...
class JColibriKafkaResourceIT {
    ...
    void setup() {
        ...
        JColibriKafkaResource kafkaResource = new JColibriKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/j-colibri-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/j-colibri-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
